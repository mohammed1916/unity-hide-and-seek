...
def main():
    ...
    positions = np.stack([ep["positions"] for ep in episodes], axis=0)
    goals = np.stack([ep["goals"] for ep in episodes], axis=0)

    # Compute velocities from diffs along the time axis
    velocities = np.diff(positions, axis=2, prepend=positions[:, :, :1, :])
    ...
    if args.dashboard:
        if enrich_with_ppo_metrics is None or plot_metrics_dashboard is None:
            ...
            ep_df2, agent_df2, sample_eff = enrich_with_ppo_metrics(
                ep_df, agent_df, positions, goals=goals
            )
            plot_metrics_dashboard(
                ep_df2, sample_eff,
